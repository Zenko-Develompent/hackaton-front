...
export default function ExamPage() {
  ...
  const [currentSection, setCurrentSection] = useState<'questions' | 'tasks'>('questions');
  ...
  const [taskResults, setTaskResults] = useState<Record<string, { completed: boolean; code?: string }>>({});
  ...
  const handleAnswerSubmit = async () => {
    ...
    if (currentQuestionIndex + 1 < questions.length) {
      ...
      // Все вопросы отвечены правильно, переходим к заданиям
  ...
  const completeExam = async () => {
    ...
    const allTasksCompleted = tasks.every(task => taskResults[task.taskId]?.completed);
  ...
  if (!exam) {
    return (
      <Container>
        <div className="flex flex-col items-center justify-center py-40">
          <p className="text-gray-500 mb-4">Экзамен не найден</p>
    ...
    return (
      <Container>
        ...
        <ResultDisplay
          ...
          details={[
            { label: "Вопросы", value: `${result.questionsDone} / ${result.questionsTotal}` },
            { label: "Задания", value: `${result.tasksDone} / ${result.tasksTotal}` },
          ...
          <div className=" text-center">
            <div className="mb-6">
              ...
              <p className="text-gray-600 mb-6">
                Вы допустили ошибку при ответе на вопрос. Для успешной сдачи экзамена все вопросы должны быть отвечены правильно.
              </p>
            ...
            <div className="space-y-4 mb-8 p-4 bg-gray-50 rounded-xl">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-green-600">
                  {examDetails?.xpReward || 0} XP + {examDetails?.coinReward || 0} монет
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Условие:</span>
                <span className="font-semibold text-orange-600">Все вопросы должны быть отвечены правильно</span>
              </div>
            </div>
  ...
  // Рендер заданий
  const allTasksCompleted = tasks.every(task => taskResults[task.taskId]?.completed);
  const completedCount = tasks.filter(task => taskResults[task.taskId]?.completed).length;
      ...
      <div className="py-10">
        ...
        {!allTasksCompleted && (
          <p className="text-center text-sm text-gray-500 mt-4">
            Выполните все задания ({tasks.length - completedCount} осталось), чтобы завершить экзамен
          </p>
        )}
      </div>
    </Container>
